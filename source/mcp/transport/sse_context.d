...
unittest  // a GET carrying Last-Event-ID replays events emitted after that cursor
{
	...
	string[] resumed;
	ch.addListener((string f) @safe { resumed ~= f; }, Json(""), ListenFilter.init, idLine);
...
unittest  // an unknown / empty Last-Event-ID falls back to a fresh stream ordinal
{
	...
	string[] bogus;
	ch.addListener((string f) @safe { bogus ~= f; }, Json(""), ListenFilter.init, "999-5");
...
unittest  // replay never crosses streams (MUST NOT replay a different stream)
{
	...
	string[] resumed;
	ch.addListener((string f) @safe { resumed ~= f; }, Json(""), ListenFilter.init, aId0);
	assert(resumed.length == 1);
...
unittest  // resume is session-scoped: another session cannot replay a stream's history
{
	...
	string[] aFrames;
	const a = ch.addListener((string f) @safe { aFrames ~= f; }, Json(""),
	...
	string[] bFrames;
	ch.addListener((string f) @safe { bFrames ~= f; }, Json(""),
	...
	string[] bLive;
	const b2 = ch.addListener((string f) @safe { bLive ~= f; }, Json(""),
			ListenFilter.init, aId0, null, "session-B");
...
unittest  // pushToSession delivers a change notification ONLY to a stream that opted in
{
	...
	// registration order.
	ch.addListener((string f) @safe { bFrame = f; }, Json("listen-B"), fb);
	ch.addListener((string f) @safe { aFrame = f; }, Json("listen-A"), fa);
...
unittest  // pushToSession for resources/updated targets only the stream with that URI
{
	...
	ch.addListener((string f) @safe { aFrame = f; }, Json("A"), fa);
	ch.addListener((string f) @safe { bFrame = f; }, Json("B"), fb);
...
unittest  // broadcast fans out to every distinct session, once each
{
	...
	auto ch = new ServerPushChannel(coord);
	string aFrame, bFrame;
	ch.addListener((string f) @safe { aFrame = f; }, Json(""), ListenFilter.init, "", null, "A");
	ch.addListener((string f) @safe { bFrame = f; }, Json(""), ListenFilter.init, "", null, "B");
...
unittest  // broadcast honours Multiple Connections within one session
{
	...
	auto ch = new ServerPushChannel(coord);
	int aCount, bCount;
	ch.addListener((string) @safe { aCount++; }, Json(""), ListenFilter.init, "", null, "S");
	ch.addListener((string) @safe { bCount++; }, Json(""), ListenFilter.init, "", null, "S");
...
unittest  // broadcast skips a session whose only stream is not eligible
{
	...
	ch.addListener((string f) @safe { aFrame = f; }, Json("listen-A"), fa, "", null, "A");
	ch.addListener((string f) @safe { bFrame = f; }, Json("listen-B"), fb, "", null, "B");
...
unittest  // MODE 1 broadcast reaches every connected session's listener
{
	...
	auto ch = new ServerPushChannel(coord);
	string a, b, c;
	ch.addListener((string f) @safe { a = f; }, Json(""), ListenFilter.init, "", null, "A");
	ch.addListener((string f) @safe { b = f; }, Json(""), ListenFilter.init, "", null, "B");
	ch.addListener((string f) @safe { c = f; }, Json(""), ListenFilter.init, "", null, "C");
...
unittest  // connectedOwnerTokens lists distinct non-empty session tokens
{
	...
	auto coord = new StreamCoordinator;
	auto ch = new ServerPushChannel(coord);
	ch.addListener((string) @safe {}, Json(""), ListenFilter.init, "", null, "A");
	ch.addListener((string) @safe {}, Json(""), ListenFilter.init, "", null, "A"); // dup token
	ch.addListener((string) @safe {}, Json(""), ListenFilter.init, "", null, "B");
...
unittest  // pushToSession: no eligible stream means no delivery (returns 0)
{
	...
	ch.addListener((string fr) @safe { frame = fr; }, Json("only"), f);
...
unittest  // a per-listener plainEligible gate overrides the mount-wide plainEligible
{
	...
	ch.addListener((string f) @safe { aFrame = f; }, Json(""), ListenFilter.init, "", &aGate);
	ch.addListener((string f) @safe { bFrame = f; }, Json(""), ListenFilter.init, "", &bGate);
...
unittest  // a notification delivered on a listen stream is stamped with its subscriptionId
{
	...
	ch.addListener((string f) @safe { listenFrame = f; }, Json("listen-9"));
...
unittest  // emitTo to a listen stream stamps subscriptionId (e.g. the leading ack)
{
	...
	auto ch = new ServerPushChannel(coord);
	string frame;
	const id = ch.addListener((string f) @safe { frame = f; }, Json("ack-id-1"));
...
unittest  // addListener cleans up its Phase 1 registration when Phase 2 replay write throws
{
	...
	try
	{
		...
		}, Json.init, ListenFilter.init, idLine);
	}
	catch (Exception)
	{
		threw = true;
