...
2026-07-28 redesign drops the stateful `initialize` handshake in favor of a
**per-request** model:
...
- **Per-request `_meta`** — every request carries the protocol version, client
  ...
  modern model rides **identically over stdio and Streamable HTTP** — the same
  client e2e verifies both.
- **`enableModern()` / `connect()`** — `enableModern()` switches the client into
  modern mode; `connect()` runs `server/discover` and selects the
  newest mutually-supported version (2026-07-28 here).
- **zero-RTT reconnect — `connect(discoverResult())`** — the first client's
  ...
  alone with **no** `server/discover` round-trip, adopting the modern session and
  the server identity directly before serving a `tools/call`.
- **`CacheableResult` freshness hints** — modern results may carry `ttlMs` /
...
- `server.d` — one binary, either transport. Defaults to **stdio**
  ...
  **Streamable HTTP** (`runStreamableHttp`). Exposes the `add` tool and the
  `demo://greeting` resource, plus the two modern cache hints. The server object
  is protocol-agnostic; 2026-07-28 model is engaged per-request by the client.
...
```
OK: modern e2e passed over <transport> — discover(2026-07-28), connect()=2026-07-28, listTools[add] cache(5000/public), add->{"sum":42} (+structuredContent), greeting resource cache(9000/private), unknown-tool=-32602, zero-RTT reconnect via connect(discoverResult).
```
